...
export default {
  ...
  methods: {
    ...
    toLogin () {
      if (sessionStorage['token'] === 'T') { // * 当前状态为已登录，点击退出
        this.logout();
        sessionStorage['token'] = 'F'; // * 退出，清空本地存储的信息
        sessionStorage['blogId'] = 0; // * 默认为0（无此数据）
        sessionStorage['userName'] = ''; // * 清空用户名
      } else {
        // * 当前状态为未登录，点击跳转到登录界面
        this.path = '/login'
      }
